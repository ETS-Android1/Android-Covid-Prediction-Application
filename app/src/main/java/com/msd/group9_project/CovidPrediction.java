...
// Prediction Activity class for all fields
//CLass and Methods to recieve the input user values and store in variables
...
//Setting Initial Count Value to Zero
    ...
    //Prediction Method for all the variables
    ...
    public void predict_method(View v) {
        ...
        if (onRadioAllCheck(v) && onallTVCheck(v)) {
            ...
            diastolic = Integer.parseInt(arrOfbp[0]);
    ...
    public void showResultDialog(){
        ...
        dialog.setContentView(R.layout.dialog_covid_prediction);
        Window window = dialog.getWindow();
        window.setLayout(RecyclerView.LayoutParams.MATCH_PARENT, RecyclerView.LayoutParams.WRAP_CONTENT);

        ImageView imgBack = dialog.findViewById(R.id.imgClose);
        TextView tvResult = dialog.findViewById(R.id.tv_predict);
        tvResult.setText(tv_predict.getText().toString());
        imgBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dialog.dismiss();
    ...
    public boolean onallTVCheck(View v) {
        age = tie_age.getText().toString().trim();
        temp = tie_temperature.getText().toString().trim();
        bp = tie_bp.getText().toString().trim();
        oxy_levels = tie_oxygen_levels.getText().toString().trim();
        cret_levels = tie_cretinine_levels.getText().toString().trim();
        if(age.isEmpty()){
            tie_age.setError("Please Enter Age");
            tie_age.setFocusable(true);
            tie_age.requestFocus();
            return  false;
        }else if(temp.isEmpty()){
            tie_temperature.setError("Please Enter Temperature");
            tie_temperature.setFocusable(true);
            tie_temperature.requestFocus();
            return  false;
        }else if(bp.isEmpty()){
            tie_bp.setError("Please Enter Blood Pressure");
            tie_bp.setFocusable(true);
            tie_bp.requestFocus();
            return  false;
        }else if(oxy_levels.isEmpty()){
            tie_oxygen_levels.setError("Please Enter Oxygen Level");
            tie_oxygen_levels.setFocusable(true);
            tie_oxygen_levels.requestFocus();
            return  false;
        }else if(cret_levels.isEmpty()){
            tie_cretinine_levels.setError("Please Enter Creatinine Level");
            tie_cretinine_levels.setFocusable(true);
            tie_cretinine_levels.requestFocus();
            return  false;
        } else if(Integer.parseInt(age)>99||Integer.parseInt(age)<1){
            tie_age.setError("Age should between 1-99 years");
            tie_age.setFocusable(true);
            tie_age.requestFocus();
            return false;
        }else if(Float.parseFloat(temp)>155||Float.parseFloat(temp)<60){
            tie_temperature.setError("Please Enter between 60-155 F");
            tie_temperature.setFocusable(true);
            tie_temperature.requestFocus();
            return false;
        }else if(!bp.contains("/")){
            tie_bp.setError("accepted format is **/**. eg. 120/80");
            tie_bp.setFocusable(true);
            tie_bp.requestFocus();
            return false;
        }else if(Integer.parseInt(oxy_levels)<0||Integer.parseInt(oxy_levels)>150){
            tie_oxygen_levels.setError("Please enter between 0-150");
            tie_oxygen_levels.setFocusable(true);
            tie_oxygen_levels.requestFocus();
            return false;
        }else if(Float.parseFloat(cret_levels)<0||Float.parseFloat(cret_levels)>15){
            tie_cretinine_levels.setError("Please enter between 0 - 10");
            tie_oxygen_levels.setFocusable(true);
            tie_oxygen_levels.requestFocus();
            return false;
        }
    ...
    public void predictResult(View v) {
        if (contaced_person.equalsIgnoreCase("yes") || cret_double >= 5 || oxy_double <= 60 || chron.equalsIgnoreCase("yes")) {
            ...
            tv_predict.setTextColor(this.getResources().getColor(R.color.white));
